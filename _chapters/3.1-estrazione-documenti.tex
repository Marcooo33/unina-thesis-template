...
\end{figure}
Grazie a questa architettura a triage personalizzata, il sistema ha dimostrato una robustezza assoluta durante la fase di testing. Ad esempio, analizzando un batch di 11 documenti eterogenei estratti dalla Provincia di Taranto — composto nello specifico da 6 documenti nativamente \textit{tagged}, 3 documenti \textit{non-tagged} a testo nativo e 2 documenti interamente scansionati — il sistema ha completato l'intero processo di estrazione in 1013.33 secondi (circa 17 minuti), senza registrare alcuna perdita di dati o fallimenti catastrofici dovuti alla memoria.

NOTE: Aggiornare le statistiche con i risultati più recenti ottenuti durante la fase di testing finale, includendo il numero totale di pagine elaborate, la distribuzione tra pagine semplici, complesse e scansionate, e il tempo medio di elaborazione per ciascuna categoria. (In teoria saranno 110 file PDF)

L'implementazione completa in linguaggio Python del \textit{Custom Triage Processor} è consultabile nell'Appendice~\ref{app:codice_sorgente} di questo elaborato.