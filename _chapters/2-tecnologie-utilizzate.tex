...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Ottima integrazione con tutte le librerie utilizzate:} Python risulta perfettamente integrato con tutti gli strumenti e le librerie adottate nell'intero stack tecnologico (dai tool di estrazione testuale ai modelli LLM e al database Neo4j tramite API native), permettendo di orchestrare l'intera pipeline in modo fluido e senza la necessità di complessi adattatori o wrapper di terze parti.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Modelli di Embedding:} La vettorializzazione del testo normativo rappresenta un elemento critico per la qualità della ricerca semantica. Sono stati confrontati molteplici modelli per osservarne il comportamento sul dominio giuridico italiano, distinguendo la completezza del recupero dalla qualità dell'ordinamento anziché assumere l'esistenza di un vincitore unico. I test più approfonditi hanno coinvolto i seguenti modelli:
    \begin{itemize}
...
Per la containerizzazione dei componenti è stato adottato \textbf{Docker}. La scelta è motivata principalmente dalla necessità di isolare le dipendenze delle diverse fasi della pipeline, che impiegano librerie e runtime eterogenei, e di garantire la riproducibilità degli ambienti di esecuzione.

L'utilizzo di container consente inoltre di sviluppare, testare e aggiornare i singoli componenti in modo indipendente, semplificando la manutenzione del sistema.
