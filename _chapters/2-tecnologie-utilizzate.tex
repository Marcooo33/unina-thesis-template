...
La scelta è ricaduta su Neo4j, il database a grafo più diffuso e maturo sul mercato, per le seguenti motivazioni: 
    \begin{itemize}
        ...
        \item \textbf{Supporto nativo per la Vector Search:} Il database permette di memorizzare vettori ad alta dimensionalità (fino a 4096 dimensioni) come proprietà dei nodi. Questa caratteristica, unita alla possibilità di creare indici vettoriali dedicati, risulta fondamentale per le operazioni di ricerca basate sulla \textit{cosine similarity}.
    \end{itemize}
...
Infine, per valorizzare le informazioni topologiche della rete, si è fatto ricorso alla \textbf{Graph Data Science Library} \cite{graph_data_science}, una libreria nativa dedicata all'implementazione di algoritmi analitici sui grafi. Al suo interno è stato selezionato \textbf{Node2Vec} \cite{node2vec}, un potente algoritmo di \textit{graph embedding}. 
