...
\begin{enumerate}
    \item \textbf{Estrazione e arricchimento contestuale:} Il sistema esegue una query Cypher (\texttt{FETCH\_QUERY}) che risale la gerarchia del documento a partire dal testo del nodo \texttt{:Paragraph}. La query estrae i metadati dei nodi padre (Atto, Titolo, Capo, Articolo) e li concatena al contenuto del paragrafo, generando una stringa arricchita e contestualizzata.
...
\subsection{Orchestrazione del Processo}
Il flusso di \textit{ingestion} è governato da uno script principale che, sfruttando un \textit{Factory Pattern}, istanzia dinamicamente tutti i modelli di embedding. Successivamente, esegue un loop iterativo: per ogni modello caricato, lancia l'esecuzione su tutte le strategie di database configurate (Neo4j e Qdrant). L'intero ciclo è tracciato minuziosamente tramite un modulo di logging su file e console per monitorare i tempi di generazione e individuare bottleneck prestazionali.