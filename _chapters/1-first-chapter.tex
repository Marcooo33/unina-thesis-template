...
\begin{itemize}
   \item \textbf{Open Data Loader:} Una libreria open-source scelta per la sua capacità di estrarre testo in modo \textit{deterministico} da file PDF, convertendolo direttamente in Markdown.
    \item \textbf{Docling:} Una libreria open-source di parsing e analisi del layout documentale, specializzato nell'estrazione precisa di testo, tabelle e metadati strutturati a partire da formati complessi come i PDF. 
    Nel contesto di questo lavoro, è stato integrato come supporto a Open Data Loader all'interno di un sistema di ``Triage Processor'' personalizzato. 
    Viene invocato dinamicamente per elaborare in modo specifico le pagine etichettate come complesse, garantendo un'estrazione fedele delle informazioni legali.
