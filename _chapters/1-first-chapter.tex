...
\subsection*{Linguaggi di Programmazione e Architettura Software}
\begin{itemize}
    \item \textbf{Python:} Scelto per la vastità e la potenza delle sue librerie, è stato il linguaggio cardine per la costruzione delle pipeline del mio progetto.
    È stato impiegato applicando design pattern architetturali (\textit{Object-Oriented Programming, Strategy Pattern, Repository Pattern, Factory Pattern}).
    \item \textbf{Docker:} L'intero ecosistema è stato containerizzato attraverso lo sviluppo di moduli isolati e specifici (es. \texttt{extract-from-pdf}, \texttt{db-ingestion}, \ldots{}) per ogni step della mia pipeline. Questa scelta ha garantito un'elevata modularità, l'isolamento delle dipendenze software e un rigoroso controllo sulle risorse computazionali, specialmente durante i task più onerosi legati all'\textbf{\textit{Optical Character Recognition (OCR)}} e alla \textbf{\textit{generazione degli Embedding}}.
\end{itemize}

\subsection*{Strumenti per l'Estrazione del Testo (PDF Parsing \& OCR)}
\begin{itemize}
    \item \textbf{Open Data Loader:} Una libreria fondamentale scelta per la sua capacità di eseguire estrazioni di testo \textit{deterministiche} da PDF complessi (\textit{Tagged PDF}), mantenendo l'ordine di lettura e la struttura ad albero originaria, convertendoli in Markdown.
    \item \textbf{Docling:} Integrato come supporto a Open Data Loader all'interno di un sistema di ``Triage Processor'' personalizzato.
    Viene invocato dinamicamente dallo script Python per elaborare le pagine etichettate come complesse o prive di un albero strutturale chiaro.
    \item \textbf{Motori OCR:} Moduli impiegati per estrarre testo dai documenti derivanti da scansioni visive.
    Tra i vari motori OCR disponibili, è stato optato l'uso di \textbf{RapidOCR}, che si è rivelato il compromesso più efficace per bilanciare i tempi di esecuzione e l'elevato consumo di RAM che portava al blocco dei container.
\end{itemize}
...
\begin{itemize}
    ...
    Riceveva in input il testo in formato Markdown estratto dai PDF e restituiva JSON formattati rigorosamente secondo uno schema predefinito, che rappresentavano la gerarchia dei documenti normativi (Titoli, Capi, Articoli, Commi) e i nodi concettuali.
    \item \textbf{Framework Sentence-Transformers:} Libreria Python utilizzata per l'embedding locale del testo dei commi, al fine di permettere la ricerca semantica.
    \item \textbf{Modelli di Embedding:} Sono stati testati svariati modelli per ottimizzare la vettorializzazione del testo legale in italiano. I test più approfonditi hanno coinvolto i seguenti modelli: \texttt{intfloat/multilingual-e5-large}, \texttt{google/gemini-embedding-001} e \texttt{BAAI/bge-m3-14b-1T}. La valutazione si è basata sulla capacità di recuperare commi pertinenti a partire da query in linguaggio naturale.
\end{itemize}